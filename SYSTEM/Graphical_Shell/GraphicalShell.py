...
launcher_button_rect = pygame.Rect(5, 4, ICON_SIZE, ICON_SIZE)
try:
...
filemgr_button_rect = pygame.Rect(launcher_button_rect.right + 10, 4, ICON_SIZE, ICON_SIZE) 
try:
...
browser_button_rect = pygame.Rect(filemgr_button_rect.right + 5, 4, ICON_SIZE, ICON_SIZE)
try:
...
terminal_button_rect = pygame.Rect(browser_button_rect.right + 5, 4, ICON_SIZE, ICON_SIZE) 
try:
...
while running:
    ...
    draw_transparent_taskbar(screen, pygame.Rect(0, 0, WIDTH, TASKBAR_HEIGHT), TASKBAR_DARK, 230)
    ...
    for p in launched_programs:
        ...
        p.taskbar_rect = pygame.Rect(current_text_x, 4, btn_w, TASKBAR_HEIGHT - 8)
    ...
    network_icon_rect = pygame.Rect(clock_x - 45, 4, ICON_SIZE, ICON_SIZE)
    ...
    screen.blit(clock_text, (clock_x, (TASKBAR_HEIGHT - clock_text.get_height()) // 2))

    dots_menu_rect = pygame.Rect(network_icon_rect.x - 35, 7, 26, 26)
    ...
    if special_menu_open:
        ...
        for i, label in enumerate(special_menu_items):
            rect = pygame.Rect(dots_menu_rect.x - 160, dots_menu_rect.bottom + 10 + i * 38, 190, 35)
            ...
            screen.blit(font.render(label, True, WHITE), (rect.x + 10, rect.y + 4))

    if menu_visible:
        items = menu_items[scroll_offset:scroll_offset + 10]
        menu_bg = pygame.Rect(5, TASKBAR_HEIGHT + 5, menu_width, len(items) * menu_item_height + 10)
        draw_rounded_transparent_rect(screen, menu_bg, DARK_BLUE, 240, 10)
        for i, item in enumerate(items):
            item_rect = pygame.Rect(5, TASKBAR_HEIGHT + 10 + i * menu_item_height, menu_width, menu_item_height)
            if item_rect.collidepoint((mouse_x, mouse_y)):
                pygame.draw.rect(screen, PRIMARY_BLUE, item_rect, border_radius=4)
            screen.blit(font.render(item, True, WHITE), (item_rect.x + 12, item_rect.y + 2))
        ...
        if event.type == pygame.QUIT: running = False
            ...
            if launcher_button_rect.collidepoint(event.pos):
                ...
                for i, item in enumerate(menu_items[scroll_offset:scroll_offset+10]):
                    if pygame.Rect(5, TASKBAR_HEIGHT + 10 + i * menu_item_height, menu_width, menu_item_height).collidepoint(event.pos):
